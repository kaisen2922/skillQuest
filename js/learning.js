...
function loadJSLesson(lessonId) {
  ...
  const notesEl = document.querySelector('.lesson-notes') || document.getElementById('lesson-notes-container');
  if (notesEl) {
    notesEl.innerHTML = lesson.concepts.map(c => `
      <div class="glass-panel lesson-note" style="padding:1.2rem; border-left:3px solid var(--neon-blue);">
        <div style="display:flex;align-items:center;gap:0.6rem;margin-bottom:0.5rem;">
          <i class="${c.icon}" style="font-size:1.3rem;color:var(--neon-blue);"></i>
          <strong style="font-family:var(--font-display);letter-spacing:1px;">${c.title}</strong>
        </div>
        <p style="font-size:0.9rem;opacity:0.85;line-height:1.65;">${c.body}</p>
      </div>
    `).join('');
  }
